...
def nlu(input=""):
    ...
    if "dialogue_state_history" in dst:
        if dst["dialogue_state_history"][-1] == "greeting":
            ...
            pattern = re.compile(r"\b([Yy]es)|([Nn]o)\b") # checks if input has yes or no in it
        ...
        if dst["dialogue_state_history"][-1] == "social_interaction":
            ...
            pattern = re.compile(r"\b([Dd]ifferent)|([Ss]ame)|([Nn]ew)|([Ff]amiliar)\b") # making sure user is answering question
            match = re.search(pattern, input)
            if match:
        ...
        if dst["dialogue_state_history"][-1] == "job_function":
            # Check to see if the input contains a valid answer.
            pattern = re.compile(r"hands|behind")
            match = re.search(pattern, input)
            if match:
        ...
        if dst["dialogue_state_history"][-1] == "imagine_analyze":
            # Check to see if the input contains a valid answer.
            pattern = re.compile(r"\b([Yy]es)|([Nn]o)|([Nn]ot)\b") # checks if input has yes or no in it
            match = re.search(pattern, input)
            if match:
        ...
        user_intent = "unknown" # otherwise, the user's intent is unknown TODO: We should probably add a case of what to do if user intent is unknown
    ...
    if user_intent == "ans_soc_inter":
        pattern = re.compile(r"\b([Dd]ifferent)|([Nn]ew)\b") # likes to interact with new people -probably good for service sector
        diff = re.search(pattern, input)
        
        pattern = re.compile(r"\b([Ss]ame)|([Ff]amiliar)\b") # prefers to stick to familiar group of people
    ...
    if user_intent == "ans_jobfunc":
        ...
        pattern = re.compile(r"behind") # should be enough to indicate they like behind the scenes kind of work.. or low key kind or work
        b = re.search(pattern, input)
        
        if h:
            slots_and_values.append(("job_function","hands_on"))
        elif b:
    ...
    if user_intent == "ans_imag_analyze":
        pattern = re.compile(r"\b([Yy]es)\b") # checks if input has yes
...
def main():
    ...
    dst_input = [("dialogue_state_history", "greeting"), ("dialogue_state_history", "social_interaction"), ("user_intent_history", "greeting")]
    update_dst(dst_input)
    print("\nCurrent Global DST:\n{0}".format(dst))
    
    # Sample user input
    user_input = "I prefer working in small groups with the same people from day to day"
    
    # Call nlu(user_input).  The function should return a list of (slot, value) pairs, containing (in this
    # case) the pizza size and the user intent.
    slots_and_values = nlu(user_input)
    print("\nNLU Output:\n{0}".format(slots_and_values))
    
    update_dst(slots_and_values)
    
    # Call get_dst().  The function should return a dictionary with keys and values as specified previously.
    ...
    print("\nNext State: {0}\tSlot Values: {1}".format(next_state, slot_values))
    
    # Call nlg(next_state, slot_values).  The function should generate a natural-sounding output given
    # the specified state and optional slot values.  Assuming that next_state = "clarification" and
    # slot_values = [("num_pizzas", 5)], the sample function will output:
    # Just double-checking ...did you say that you want 5 pizzas?
    output = nlg(next_state, slot_values)
    print("\nOutput: {0}\n".format(output))

    # Test Cases
    # TEST CASE # 1
    output = nlu("I like meeting new people every day")
    print("Test Case #1: {0}".format(output))
    # Explanation: This test case works, because the chatbot understands that the user likes interacting with different people and adds "different people" for social interaction
    
    update_dst([("dialogue_state_history","intro_extroversion")])
    
    # TEST CASE # 2
    output = nlu("I think I'm an extrovert really")
    print("Test Case #2: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user is an extrovert
    
    # TEST CASE # 3
    output = nlu("What is an extrovert?")
    print("Test Case #3: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user is asking for a definition of an extrovert
    
    update_dst([("dialogue_state_history","right_leftbrain")])
    
    # TEST CASE # 4
    output = nlu("Oh, I’m more likely to follow my heart")
    print("Test Case #4: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user is a right-brain individual

    # TEST CASE # 5
    output = nlu("I live in the moment")
    print("Test Case #5: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that "in-the-moment" is right-brain

    # TEST CASE # 6
    output = nlu("I actually like to analyze things closely")
    print("Test Case #6: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user analyzing things closely makes them left-brain
    
    update_dst([("dialogue_state_history","likes_history")])

    # TEST CASE # 7
    output = nlu("Oh, I love history! I’m kind of a history buff!")
    print("Test Case #7: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user likes history
    # TEST CASE # 8
    output = nlu("Oh no, I'm not a fan of history!")
    print("Test Case #8: {0}".format(output))
    # Explanation: This test case does not pass, because the chatbot thinks the user likes history when, in fact, they siad they don't like it. Perhaps a way to fix this is to look for negations like 'not' or 'don't' that might flip the meaning of this. I can do this later using NLTK python package
    
    update_dst([("dialogue_state_history","job_function")])

    # TEST CASE # 9
    output = nlu("I work the best with a hands-on task")
    print("Test Case #9: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user likes hands-on work

    # TEST CASE # 10
    output = nlu("I generally prefer to be behind-the-scenes")
    print("Test Case #10: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user likes "behind-the-scenes" jobs over hands-on work
    
    update_dst([("dialogue_state_history","innovate")])
    
    # TEST CASE # 11
    output = nlu("Why fix what’s not broken? I like sticking to the proven way of doing things")
    print("Test Case #11: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user prefers to do things the "old-fashioned" way

    # TEST CASE # 12
    output = nlu("I always like to be creative when I can be.")
    print("Test Case #12: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user is creative
    
    update_dst([("dialogue_state_history","empathy")])
    
    # TEST CASE # 13
    output = nlu("No, that's not really my style")
    print("Test Case #13: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user is not empathetic
    
    update_dst([("dialogue_state_history","result")])
    
    # TEST CASE # 14
    output = nlu("Ok cool. Thanks for the suggestion")
    print("Test Case #14: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user acknowledged the career results
    
    update_dst([("dialogue_state_history","imagine_analyze")])
    
    # TEST CASE # 15
    output = nlu("No... I live in the present")
    print("Test Case #15: {0}".format(output))
    # Explanation: This test case passes, because the chatbot successfully understands that the user is not imaginative, or analytical of the future.
